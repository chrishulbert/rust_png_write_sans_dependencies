...
// Test this by running `cargo run > test.png`
fn main() {    
    const W: u32 = 0xffefe4ff; // White-ish.
    const R: u32 = 0xfb0351ff; // Red.
    const B: u32 = 0x000000ff; // Black.
    const L: u32 = 0x14dd59ff; // Light green.
    const G: u32 = 0x008641ff; // Dark green.
    const C: u32 = 0x00000000; // Clear.
    let watermelon = png(8, 6, &[
        W,R,B,R,R,R,R,W,
        W,R,R,R,R,B,R,W,
        W,R,R,B,R,R,R,W,
        L,W,R,R,R,R,W,L,
    ...
    stdout.write_all(&watermelon).unwrap();
}
